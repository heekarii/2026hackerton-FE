        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[360px_minmax(0,1fr)]">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {visibleHotspots.map((spot) => <button key={spot.name} type="button" onClick={() => setSelectedHotspot(spot)} className={cn('rounded-md border px-3 py-2 text-xs font-bold', activeHotspot?.name === spot.name ? 'border-red-700 bg-red-50 text-red-800' : 'border-slate-200 text-slate-600 hover:bg-slate-50')}>{spot.name} {spot.count}건</button>)}
            </div>
          </section>
        </div>
      </section>
